...
def getModel(modelName: str):
    if modelName == "vgg_unet":
        return vgg_unet(n_classes=2 ,  input_height=INP_HEIGHT, input_width=INP_WIDTH)
    if modelName == "unet":
        return unet(n_classes=2 ,  input_height=INP_HEIGHT, input_width=INP_WIDTH)
    if modelName == "resnet50_unet":
        return resnet50_unet(n_classes=2 ,  input_height=INP_HEIGHT, input_width=INP_WIDTH)
    if modelName == "segnet":
        return segnet(n_classes=2 ,  input_height=INP_HEIGHT, input_width=INP_WIDTH)
